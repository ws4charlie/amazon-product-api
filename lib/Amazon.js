...
class AmazonScraper {
    ...
    get setRequestEndpoint() {
        switch (this.scrapeType) {
            case 'products':
                return 's';
            case 'reviews':
                return `product-reviews/${this.asin}/ref=cm_cr_arp_d_viewopt_rvwer?formatType=${
                    CONST.reviewFilter.formatType[this.reviewFilter.formatType]
                }&sortBy=${CONST.reviewFilter.sortBy[this.reviewFilter.sortBy] ? CONST.reviewFilter.sortBy[this.reviewFilter.sortBy] : ''}&pageNumber=1${
            ...
            // case 'verify':
            //     return `product-reviews/${this.asin}/ref=cm_cr_arp_d_viewopt_rvwer?${
            //         this.reviewFilter.verifiedPurchaseOnly ? 'reviewerType=avp_only_reviews' : ''
            //     }&pageNumber=1&sortBy=${CONST.reviewFilter.sortBy[this.reviewFilter.sortBy] ? CONST.reviewFilter.sortBy[this.reviewFilter.sortBy] : ''}${
            //         this.reviewFilter.filterByStar ? `&filterByStar=${CONST.reviewFilter.filterByStar[this.reviewFilter.filterByStar]}` : ''}`;
            case 'verify':
                return "reviews/product/17017931";
            case 'asin':
    ...
    grabReviews(body) {
        ...
        for (let key in scrapingResult) {
            ...
            try {
                const avpBadge = $(search[0]).text().toLowerCase();
                scrapingResult[key].verified_purchase = avpBadge.includes('verified purchase');
            } catch (error) {
                continue;
            }
        }
    ...
    /**
     * Get review by hash from the html response
     * @param {} body
     */
    grabReviewByHash(body) {
        var status = CONST.status.NOTFOUND;
        const $ = cheerio.load(body.replace(/\s\s+/g, '').replace(/\n/g, ''));
        console.log('grabReviewByHash: ');
        ...
        for (let i = 0; i < reviewsList.length; i++) {
            const totalInResult = Object.keys(scrapingResult).length + this.collector.length;
            //console.log("totalInResult: " + totalInResult);
            if (totalInResult >= this.number && this.bulk) {
                break;
            }
            const reviewId = reviewsList[i].attribs['id'];
        ...
        // Loop through the list of reviews and extract data
        var review = {};
        for (let key in scrapingResult) {
            // If purchase is verified
            const search = $(`#${key} [data-hook="avp-badge"]`);
            try {
                const avpBadge = $(search[0]).text().toLowerCase();
                if (!avpBadge.includes('verified purchase')) {
                    continue;
                }
            } catch (error) {
                continue;
            ...
            // Review date
            const search_date = $(`#${key} [data-hook="review-date"]`);
            try {
                const review_date = search_date[0].children[0].data;
                if (review_date) {
                    const date = this.geo.review_date(review_date);
                    //console.log('unix: ', date.unix, 'submissionTime: ', this.submissionTime);
                    if (date.unix < this.submissionTime) {
                        break;
                    }
                    review.submissionTime = date.unix;
                }
            } catch (error) {
            ...
            // Reviewer name
            const search_name = $(`#${key} .a-profile-name`);
            try {
                var name = search_name[0].children[0].data;
            ...
            // Rating
            const search_rating = $(`#${key} [data-hook="review-star-rating"]`);
            try {
                review.rating = parseFloat(search_rating[0].children[0].children[0].data.split(' ')[0]).toFixed(1);
            } catch (error) {
                continue;
            }

            // Review title
            const search_title = $(`#${key} [data-hook="review-title"]`);
            try {
                review.title = $(search_title[0]).text().toString();
            } catch (error) {
                continue;
            }

            // Review text
            const search_text = $(`#${key} [data-hook="review-body"]`);
            try {
                review.text = $(search_text[0]).text();
            } catch (error) {
                continue;
            }

            // Calculate kecak hash of a review record [name + rating + title + text]
            const record = `${review.name} ${review.rating} ${review.title} ${review.text}`;
            const hash = keccak256(record).toString('hex');
            //console.log('record: ', record);
            //console.log('hash: ', hash);
            if (hash === this.hash) {
                status = CONST.status.VERIFIED;
                console.log(`time: ${review.submissionTime}, rating: ${review.rating}, name: ${review.name}, hash: ${hash}, title: ${review.title}`);
                break;
            }
        }
    ...
    /**
     * Verify review by hash
     * @param {} body
     */
    verifyReviewByHash(body) {
        this.grabReviewByHash(body);
        return true;
    }
