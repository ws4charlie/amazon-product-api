...
require('yargs')
    ...
    .example(`$0 verify B0BMB9S1V6 621cc65087769f0ecdd2457b0973415d6bd62375dc86769615545d7658db8516`)
    ...
    .command('verify [id] [time] [hash]', 'verify review by ASIN id, time and review hash', {}, (argv) => {
        startScraper(argv);
    })
    ...
    .check((argv) => {
        if (['products', 'reviews', 'verify', 'asin', 'categories', 'countries'].indexOf(argv['_'][0]) === -1) {
            throw 'Wrong command';
        }
        ...
        if (argv['_'][0] === 'verify') {
            ...
            if (!argv.time) {
                throw 'Submission time is missing';
            } else {
                argv.submissionTime = argv.time;
            }
            if (!argv.hash) {
                throw 'Hash is missing';
            }
        }
